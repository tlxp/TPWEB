...
module.exports = function(User, SolarPanel, Certificate, Credit, gridFSBucket, upload) {
  ...
  router.get('/download-certificate', authorize(['Cliente']), async (req, res) => {
    ...
    try {
      const panel = await SolarPanel.findOne({ panelId, clientId: req.user.id });
      if (!panel) {
        return res.status(404).json({ message: `Painel com ID ${panelId} não encontrado ou não pertence ao usuário.` });
      }
      const certificate = await Certificate.findOne({ panelId: panel._id });
      if (!certificate) {
        return res.status(404).json({ message: `Nenhum certificado encontrado para o painel ${panelId}.` });
      }
      const downloadStream = gridFSBucket.openDownloadStream(certificate.certificateFileId);
      res.set('Content-Type', 'application/pdf');
      res.set('Content-Disposition', `attachment; filename="certificate_${panelId}.pdf"`);
  ...
  router.get('/user-panels', authorize(['Cliente']), async (req, res) => {
    try {
      const panels = await SolarPanel.find({ clientId: req.user.id })
        ...
        .populate('clientId', 'username');
      console.log(`Painéis encontrados para o cliente ${req.user.id}: ${panels.length}`);
      return res.json(panels);
    } catch (err) {
  ...
  router.get('/monitor-production', authorize(['Gestor Operações']), async (req, res) => {
    ...
    if (!mongoose.Types.ObjectId.isValid(clientId)) {
      return res.status(400).json({ message: 'O clientId deve ser um ObjectId válido (24 caracteres hexadecimais).' });
    }

    try {
      const client = await User.findById(clientId);
      if (!client || client.role !== 'Cliente') {
        return res.status(404).json({ message: 'Cliente não encontrado ou não é um cliente válido.' });
      }
      ...
      console.log(`Produção de energia registrada para o cliente ${clientId}: ${kWh} kWh`);
  ...
  router.get('/credits', authorize(['Gestor Operações']), async (req, res) => {
    ...
    try {
      ...
      console.log(`Créditos contabilizados para o cliente ${clientId}: ${totalCredits} kWh`);
