document.addEventListener('DOMContentLoaded', async () => {
  ...
  panelForm.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      panelForm.reset();
      concelhoSelect.innerHTML = '<option value="">Selecione um distrito primeiro</option>';
  ...
  window.downloadPanelCertificate = async (panelId) => {
    try {
      const response = await fetch(`http://localhost:3000/auth/download-certificate?panelId=${encodeURIComponent(panelId)}`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (!response.ok) {
        const error = await response.json();
        throw new Error(error.message || 'Ainda não há certificado disponível para este painel.');
      }
    ...
    try {
      ...
      if (query) {
        panels = panels.filter(panel => 
          panel.panelId.toLowerCase() === query.toLowerCase()
        );
      }

      if (panels.length === 0) {
        myPanelsResults.innerHTML = '<p class="card">Nenhum painel encontrado.</p>';
        return;
      }
  ...
  searchMyPanelsForm.addEventListener('submit', async (e) => {
    e.preventDefault();
    const query = document.getElementById('search-my-panels-query').value.trim();
    if (query && !/^PANEL\d{3}$/.test(query)) {
      myPanelsResults.innerHTML = '<p class="card error">O ID do painel deve seguir o formato PANELXXX (ex: PANEL123).</p>';
      return;
    }
